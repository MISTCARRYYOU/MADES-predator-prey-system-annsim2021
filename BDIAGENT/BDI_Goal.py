...
from BDI_struct_for_cpp import goal2schedule, overallParameters
from Message import Message


# Goal部分
class Goal(AtomicDEVS):
    def __init__(self, args, agent_id_num):
        AtomicDEVS.__init__(self, "predator-goal")
        ...
        self.overallparameters = overallParameters(args, agent_id_num)  # overallParameters类，问题在这句话上
    ...
    def _decide_plan(self):
        ...
        # 训练部分
        if self.first:
            self.deal_plan_list = [["predator-plan5"]]
            self.first = False
        else:
            self.deal_plan_list = [["predator-plan2"], ["predator-plan3"], ["predator-plan1"]]

        # 测试部分-先执行测试部分的内容
        # if self.first:
        #     self.deal_plan_list = [["plan5"]]
        #     self.first = False
        # else:
        #     self.deal_plan_list = [["plan2"], ["plan4"]]
