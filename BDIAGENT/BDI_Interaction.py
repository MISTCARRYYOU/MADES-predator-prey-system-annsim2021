...
from BDI_struct_for_cpp import interaction2plan
...
class Interaction(AtomicDEVS):
    def __init__(self, agentID):
        AtomicDEVS.__init__(self, "predator-interaction")
        ...
        if current_port == self.inport_plan:  # 来自plan的输入
            ...
            if self.message.protocol == "receive-1": # 此时不发送，只等待接收
                ...
                if self.mail_box != []:
                    ...
                    self.interaction2plan.perception = self.check_sender_forreceive2(self.sender_list, self.interaction2plan.perception)
