...
def plot_one_wave(atomic_name, processed_list, y_position, x_lim, time_base=0.0):
    ...
    for eve in related_info:
        ...
        if eve[1] == True:  # 绘制外部事件
            ...
            plt.arrow(tmp_crt,y_exte+0.3,0,0.1, color=color1, linestyle=linestyle1, head_width=0.1, lw=0.8,#箭头⻓度，箭尾线宽
    length_includes_head = True)
        else:  # 绘制内部事件

            plt.arrow(tmp_crt,y_inte+0.1, 0, -0.1, color=color2, linestyle=linestyle2, head_width=0.1, lw=0.8,#箭头⻓度，箭尾线宽
    length_includes_head = True)
...
if __name__ == "__main__":
    ...
    # 图画的有问题，还得继续改进
    # plt.legend()
    plt.text(5.19, 12.0, "Internal event transition occurs", fontsize=15)
    plt.text(5.19, 11.5, "External event transition occurs", fontsize=15)

    plt.text(0-0.4, 13.1, "training step i", fontsize=15)
    plt.text(5.1-0.4, 13.1, "training step i+1", fontsize=15)
    plt.text(10.2-0.4, 13.1, "training step i+2", fontsize=15)
    ...
    plt.xlim(xlim)
    plt.ylim([0.5, 13])
    plt.plot([5.1,5.1],[0, 13], color="r", linestyle=":")
    plt.plot([0,0],[0, 13], color="r", linestyle=":")
    plt.plot([10.2,10.2],[0, 13], color="r", linestyle=":")
    ...
    # plt.legend()
    # plt.ylim()
    # plt.savefig(r"G:\paper4\初稿\ANNSIM初稿_within12pages\manuscript\figure8.eps", format="eps")
    plt.show()
